...
Dodat ćemo dva dugma `<Button>` elementa:
  - jedno s nazivom "Dodaj"
  - drugo s nazivom "Resetiraj"

Dodati text `<TextView>` elemente "Ukupno kalorija" i "0".
...
- **Atribut:**
  - `foodList` - lista objekata tipa `FoodItem`, u kojoj se pohranjuju svi dodani prehrambeni proizvodi
- **Metode:**
  ...
  - `getAllFood()` - vraća cijelu listu svih dodanih prehrambenih proizvoda
...
EditText nazivHraneInput = findViewById(R.id.NazivHraneInput);
EditText kalorijeInput = findViewById(R.id.KalorijeInput);
Button buttonDodaj = findViewById(R.id.buttonDodaj);
Button buttonResetiraj = findViewById(R.id.buttonResetiraj);
...
buttonDodaj.setOnClickListener(view -> {
  ...
  String name = nazivHraneInput.getText().toString();
  String calText = kalorijeInput.getText().toString();

  if (!name.isEmpty() && !calText.isEmpty()) {
      int calories = Integer.parseInt(calText);
      // Adding the food info to the model
      tracker.addFood(name, calories);

      // Reading new state from the model
      int total = tracker.getTotalCalories();

      StringBuilder foods = new StringBuilder();

      // Reading foods from model since the state has changed
      for (FoodItem item : tracker.getAllFood()) {
          foods.append("- ").append(item.getName())
                  .append(": ").append(item.getCalories())
                  .append(" kcal\n");
      }

      textTotal.setText(getString(total));
      textList.setText(foods.toString());
    
      nazivHraneInput.setText("");
      kalorijeInput.setText("");
  } else {
